...
{
	return manhattanDist(toNode->pos.first, fromNode->pos.first, toNode->pos.second, fromNode->pos.second);
}

float SearchLevel::getH(SearchNode * fromPos, SearchNode * toPos)
{
	
	return euclideanDist(toPos->pos.first, fromPos->pos.first, toPos->pos.second, fromPos->pos.second);
}
...
std::vector<Position> SearchLevel::getAdjacentNodes(int posX, int posY)
{
	...
	if (isWalkable(posX, posY + 1)) // Left
		temp.push_back(Position(posX - 1, posY));	

	if (isWalkable(posX, posY + 1)) // Right
		temp.push_back(Position(posX + 1, posY));	
